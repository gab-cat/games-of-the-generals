  ...
  handler: async (ctx, args) => {
    ...
    if (paginationOpts && paginationOpts.cursor) {
      ...
      return {
        ...
        continueCursor:
          lobbies.length > 0 ? lobbies[lobbies.length - 1]._id : "",
      };
    }
  },
});
...
export const createLobby = mutation({
  args: {
    ...
    gameMode: v.optional(
      v.union(v.literal("classic"), v.literal("blitz"), v.literal("reveal")),
  ...
  handler: async (ctx, args) => {
    ...
    const existingLobby = await ctx.db
      ...
      .withIndex("by_host_status", (q) =>
        q.eq("hostId", userId).eq("status", "waiting"),
      )
      .unique();
    ...
    if (!existingLobby) {
      const playingLobby = await ctx.db
        .query("lobbies")
        .withIndex("by_host_status", (q) =>
          q.eq("hostId", userId).eq("status", "playing"),
        )
        .unique();
    ...
    if (isPrivate) {
      ...
      if (limit !== Infinity && todayCount >= limit) {
        ...
        throw new Error(
          `Daily limit of ${limit} private lobbies reached for ${tierName} tier. ${tier === "free" ? "Upgrade to Pro for 50 per day, or Pro+ for unlimited." : tier === "pro" ? "Upgrade to Pro+ for unlimited private lobbies." : ""}`,
        );
      }
    }
    ...
    // Note: Usage counter is now updated in games.startGame when game actually starts,
    // not at lobby creation time. This ensures private lobby usage is only consumed
    // when a game actually starts from the lobby.
...
export const spectateGameById = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!userIsAdmin) {
      ...
      if (
        lobby.maxSpectators &&
        game.spectators.length >= lobby.maxSpectators
      ) {
...
export const checkLobbyAbandonment = mutation({
  ...
  handler: async (ctx, { lobbyId }) => {
    ...
    const playerLastActive =
      lobby.playerLastActiveAt || (lobby.playerId ? lobby.createdAt : null);

    const hostInactive = now - hostLastActive > LOBBY_INACTIVITY_THRESHOLD_MS;
    const playerInactive =
      lobby.playerId && playerLastActive
