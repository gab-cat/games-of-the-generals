  ...
  handler: httpAction(async (ctx, request) => {
    try {
      ...
      const parsedSignature =
        paymongoModule.parsePayMongoSignature(signatureHeader);
      ...
      if (isNaN(timestamp) || Math.abs(now - timestamp) > tolerance) {
        console.error(
          "[Webhook] Timestamp outside of tolerance window:",
          parsedSignature.timestamp,
        );
        return new Response("Timestamp outside of tolerance window", {
          status: 400,
        });
      }

      console.log(
        "[Webhook] Parsed signature - timestamp:",
        parsedSignature.timestamp,
      );
      console.log(
        "[Webhook] Parsed signature - has test sig:",
        !!parsedSignature.testSignature,
      );
      console.log(
        "[Webhook] Parsed signature - has live sig:",
      ...
      console.log(
        "[Webhook] Signature verified successfully. Mode:",
      ...
      if (eventId) {
        const alreadyProcessed = await ctx.runQuery(
          internal.subscriptions.checkWebhookEvent,
          { eventId },
        );
        if (alreadyProcessed) {
          console.log(
            "[Webhook] Event already processed, returning early:",
            eventId,
          );
          return new Response(
      ...
      if (!paymentAttributes || !paymentData) {
        console.error(
          "[Webhook] Invalid event structure - missing payment data",
        );
        // Return 200 to acknowledge but log as error
        return new Response(
          JSON.stringify({ received: true, error: "Invalid event structure" }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
      ...
      const isSuccessEvent =
        eventType === "payment.paid" || eventType === "payment.succeeded";
      ...
      if (isSuccessEvent) {
        ...
        // Find payment record
        let payment = await ctx.runQuery(
          internal.subscriptions.findPaymentByPayMongoId,
          {
        ...
        if (!payment && paymentIntentId) {
          payment = await ctx.runQuery(
            internal.subscriptions.findPaymentByPayMongoIntentId,
            {
        ...
        if (!payment && metadata.paymentId) {
          ...
          const paymentDoc = await ctx.runQuery(
            internal.subscriptions.getPaymentById,
            {
          ...
          if (paymentDoc) {
            payment = paymentDoc;
            console.log(
              "[Webhook] Found payment via metadata, updating with PayMongo IDs",
            );
        ...
        // Check for donation
        let donation = await ctx.runQuery(
          internal.subscriptions.findDonationByPayMongoId,
          {
            paymongoPaymentId: paymentId,
          },
        );
        ...
        if (!donation && metadata.donationId) {
          ...
          const donationDoc = await ctx.runQuery(
            internal.subscriptions.getDonationById,
            {
          ...
          if (donationDoc) {
            ...
            console.log(
              "[Webhook] Found donation via metadata, updating with PayMongo ID",
            );
            await ctx.runMutation(
